...
const { Song, User, UserLikedSong } = require('../models');
...
router.post('/:id/like', auth, async (req, res) => {
    try {
        ...
        const existingLike = await UserLikedSong.findOne({
            where: {
                userId: req.user.id,
                songId: song.id
            }
        ...
        if (existingLike) {
            ...
            const likedSongs = await req.user.getLikedSongs();
            res.json({ liked: false, likedSongs: likedSongs.map(s => s.id) });
        } else {
            // Like
            ...
            const likedSongs = await req.user.getLikedSongs();
            res.json({ liked: true, likedSongs: likedSongs.map(s => s.id) });
        }
    } catch (error) {
